...
if(!isleaf(s)&&s->child[s->c+1])s->child[s->c+1]->pt=s;
for(int j=0;j<=t->c;j++)
{if(j!=t->c)t->data[j]=t->data[j+1];
if(!isleaf(t))t->child[j]=t->child[j+1];
if(!isleaf(t)&&t->child[j])t->child[j]->pt=t;}
t->c--;
}
...
if(!isleaf(s)&&s->child[j+1])s->child[j+1]->pt=s;}
s->data[0]=p->data[i];
s->c++;
...
if(!isleaf(t))t->child[t->c-1]->pt=t;}
else 
{t->child[t->c]=s->child[i];
if(!isleaf(t))t->child[t->c]->pt=t;}}
j=index(p,t->data[0]);
if(j>k)
for(i=k;i<p->c;i++)
